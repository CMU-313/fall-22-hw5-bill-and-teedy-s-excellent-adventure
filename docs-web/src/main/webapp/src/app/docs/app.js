 ...
 * Configuring modules.
 */
	.config(($locationProvider, $urlRouterProvider, $stateProvider, $httpProvider, $qProvider,
		...
		$httpProvider.defaults.transformRequest = [function (data) {
			var parameter = function (object) {
				let query = '';
				let name; let value; let fullSubName; let subName; let subValue; let innerObject; let i;
				...
				return query.length > 0 ? query.slice(0, Math.max(0, query.length - 1)) : query;
			};

			return angular.isObject(data) && String(data) !== '[object File]' ? parameter(data) : data;
		}];
 ...
 */
	.run(($rootScope, $state, $stateParameters, Restangular) => {
		$rootScope.$state = $state;
		$rootScope.$stateParams = $stateParameters;
...
if (location.search.includes('protractor')) {
