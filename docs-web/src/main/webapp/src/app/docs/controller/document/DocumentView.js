 ...
 */
angular.module('docs').controller('DocumentView', ($scope, $rootScope, $state, $stateParameters, $location, $dialog, $uibModal, Restangular, $translate) => {
	// Load document data from server
	Restangular.one('document', $stateParameters.id).get().then(data => {
		$scope.document = data;
	}, error => {
		$scope.error = error;
	});
	...
	Restangular.one('comment', $stateParameters.id).get().then(data => {
   ...
   */
	$scope.deleteComment = function (comment) {
		...
		const message = $translate.instant('document.view.delete_comment_message');
		const btns = [
   ...
   */
	$scope.deleteDocument = function (document) {
		...
		const message = $translate.instant('document.view.delete_document_message');
		const btns = [
			{result: 'cancel', label: $translate.instant('cancel')},
			{result: 'ok', label: $translate.instant('ok'), cssClass: 'btn-primary'},
		];
   ...
   */
	$scope.showShare = function (share) {
		// Show the link
		const link = $location.absUrl().replace($location.path(), '').replace('#', '') + 'share.html#/share/' + $stateParameters.id + '/' + share.id;
		const title = $translate.instant('document.view.shared_document_title');
		const message = $translate.instant('document.view.shared_document_message', {link});
		const btns = [
   ...
   */
	$scope.validateWorkflow = function (transition) {
		Restangular.one('route').post('validate', {
			...
			const message = $translate.instant('document.view.workflow_validated_message');
