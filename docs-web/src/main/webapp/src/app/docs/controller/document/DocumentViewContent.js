 ...
 */
angular.module('docs').controller('DocumentViewContent', ($scope, $rootScope, $stateParameters, Restangular, $dialog, $state, Upload, $translate, $uibModal) => {
   ...
   */
	$scope.deleteFile = function (file) {
		...
		const message = $translate.instant('document.view.content.delete_file_message');
		const btns = [
   ...
   */
	$scope.uploadFile = function (file, newfile, previousFileId) {
		...
		return Upload.upload({
			...
			.progress(e => {
				newfile.progress = Number.parseInt(100 * e.loaded / e.total);
			})
			.success(data => {
