 ...
 * Login controller.
 */
angular.module('docs').controller('Login', (Restangular, $scope, $rootScope, $state, $stateParameters, $dialog, User, $translate, $uibModal) => {
	...
	$scope.login = function () {
		User.login($scope.user).then(() => {
			...
			if ($stateParameters.redirectState !== undefined && $stateParameters.redirectParams !== undefined) {
				$state.go($stateParameters.redirectState, JSON.parse($stateParameters.redirectParams))
					.catch(() => {
	...
	$scope.openPasswordLost = function () {
		$uibModal.open({
			...
			Restangular.one('user').post('password_lost', {
				...
				const message = $translate.instant('login.password_lost_sent_message', {username});
				const btns = [{result: 'ok', label: $translate.instant('ok'), cssClass: 'btn-primary'}];
				$dialog.messageBox(title, message, btns);
			}, () => {
				const title = $translate.instant('login.password_lost_error_title');
				const message = $translate.instant('login.password_lost_error_message');
				const btns = [{result: 'ok', label: $translate.instant('ok'), cssClass: 'btn-primary'}];
				$dialog.messageBox(title, message, btns);
			});
		});
