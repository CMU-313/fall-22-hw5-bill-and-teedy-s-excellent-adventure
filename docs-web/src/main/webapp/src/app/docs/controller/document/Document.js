...
angular.module('docs').controller('Document', ($scope, $rootScope, $timeout, $state, Restangular, $q, $filter, $uibModal) => {
   ...
   */
	$scope.$watch('currentPage', (previous, next) => {
		if (previous === next) {
			return;
		}

		$scope.offset = ($scope.currentPage - 1) * Number.parseInt($scope.limit);
		$scope.pageDocuments();
	});
   ...
   */
	$scope.sortDocuments = function (sortColumn) {
		$scope.asc = sortColumn === $scope.sortColumn ? !$scope.asc : true;
   ...
   */
	$scope.$watch('limit', (next, previous) => {
   ...
   */
	$scope.extractNavigatedTag = function () {
		...
		$scope.navigatedFilter = $scope.navigatedTag ? {parent: $scope.navigatedTag.id} : {parent: ''};
	};
