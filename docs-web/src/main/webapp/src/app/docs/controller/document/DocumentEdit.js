 ...
 */
angular.module('docs').controller('DocumentEdit', ($rootScope, $scope, $q, $http, $state, $stateParameters, Restangular, $translate) => {
	// Alerts
	$scope.alerts = [];
	...
	// Orphan files to add
	$scope.orphanFiles = [];
	if ($stateParameters.files) {
		$scope.orphanFiles = _.isArray($stateParameters.files) ? $stateParameters.files : [$stateParameters.files];
	}
   ...
   */
	$scope.edit = function () {
		...
		document.metadata_value = _.map(document.metadata_value, value => {
		...
		// Send to server
		promise = $scope.isEdit() ? Restangular.one('document', $stateParameters.id).post('', document) : Restangular.one('document').put(document);
   ...
   */
	$scope.cancel = function () {
		if ($scope.isEdit()) {
			$state.go('document.view', {id: $stateParameters.id});
		} else {
