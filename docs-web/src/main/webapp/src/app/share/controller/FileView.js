 ...
 */
angular.module('share').controller('FileView', ($uibModal, $state, $stateParameters, $timeout) => {
	...
	modal.result.then(() => {
		...
		$timeout(() => {
			...
			if ($state.current.name === 'share.file') {
				$state.go('share', {documentId: $stateParameters.documentId, shareId: $stateParameters.shareId});
			}
		});
	});
});
