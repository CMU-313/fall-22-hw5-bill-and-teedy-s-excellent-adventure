...
(function (factory) {
   ...
   */
	...
	$.cleanData = (function (orig) {
		return function (elems) {
			let events; let element; let i;
			for (i = 0; (element = elems[i]) != null; i++) {
				try {
	...
	$.widget = function (name, base, prototype) {
		...
		$.expr[':'][fullName.toLowerCase()] = function (element) {
			return Boolean($.data(element, fullName));
		};
		...
		$.each(prototype, (prop, value) => {
			...
			proxiedPrototype[prop] = (function () {
				function _super() {
					return Reflect.apply(base.prototype[prop], this, arguments);
				}
				...
				return function () {
					...
					returnValue = Reflect.apply(value, this, arguments);
   ...
   */
	...
	const data = $.extend($.expr[':'], {
		data: $.expr.createPseudo
			? $.expr.createPseudo(dataName => function (element) {
				return Boolean($.data(element, dataName));
			})

		// Support: jQuery <1.8
			: function (element, i, match) {
				return Boolean($.data(element, match[3]));
			},
	});
   ...
   */
	...
	const ie = $.ui.ie = Boolean(/msie [\w.]+/.test(navigator.userAgent.toLowerCase()));
   ...
   */
	...
	const widgetsMouse = $.widget('ui.mouse', {
		...
		_mouseDown(event) {
			...
			const elementIsCancel = (typeof this.options.cancel === 'string' && event.target.nodeName
				? $(event.target).closest(this.options.cancel).length : false);
			if (!btnIsLeft || elementIsCancel || !this._mouseCapture(event)) {
				return true;
			}
	...
	const widgetsSortable = $.widget('ui.sortable', $.ui.mouse, {
       ...
       */
		...
		_mouseStop(event, noPropagation) {
            ...
            + (this.offsetParent[0] === this.document[0].body
				...
				$(this.helper).animate(
					animation,
					Number.parseInt(this.options.revert, 10) || 500,
					() => {
						that._clear(event);
					},
				);
		...
		toArray(o) {
			...
			items.each(function () {
				returnValue.push($(o.item || this).attr(o.attribute || 'id') || '');
			});
			return returnValue;
		},
		...
		refreshPositions(fast) {
			...
			this.floating = this.items.length > 0
		...
		_createHelper(event) {
			...
			if (helper.parents('body').length === 0) {
				$(o.appendTo !== 'parent'
					? o.appendTo
					: this.currentItem[0].parentNode)[0].append(helper[0]);
			}
		...
		_adjustOffsetFromHelper(object) {
			if (typeof object === 'string') {
				object = object.split(' ');
			}

			if ($.isArray(object)) {
				object = {left: Number(object[0]), top: Number(object[1]) || 0};
			}

			if ('left' in object) {
				this.offset.click.left = object.left + this.margins.left;
			}

			if ('right' in object) {
				this.offset.click.left = this.helperProportions.width - object.right + this.margins.left;
			}

			if ('top' in object) {
				this.offset.click.top = object.top + this.margins.top;
			}

			if ('bottom' in object) {
				this.offset.click.top = this.helperProportions.height - object.bottom + this.margins.top;
			}
		},
		...
		_getRelativeOffset() {
			if (this.cssPosition === 'relative') {
				...
				return {
					top: p.top - (Number.parseInt(this.helper.css('top'), 10) || 0)
          + this.scrollParent.scrollTop(),
					left: p.left - (Number.parseInt(this.helper.css('left'), 10) || 0)
		...
		_setContainment() {
			...
			if (!(/^(document|window|parent)$/).test(o.containment)) {
				...
				this.containment = [
					co.left + (Number.parseInt($(ce).css('borderLeftWidth'), 10) || 0)
          + (Number.parseInt($(ce).css('paddingLeft'), 10) || 0) - this.margins.left,
					co.top + (Number.parseInt($(ce).css('borderTopWidth'), 10) || 0)
          + (Number.parseInt($(ce).css('paddingTop'), 10) || 0) - this.margins.top,
       ...
       */
		...
		_clear(event, noPropagation) {
			...
			// reappended (see #4088)
			if (!this._noFinalSort && this.currentItem.parent().length > 0) {
		...
		_trigger() {
			if (Reflect.apply($.Widget.prototype._trigger, this, arguments) === false) {
				this.cancel();
			}
		},
