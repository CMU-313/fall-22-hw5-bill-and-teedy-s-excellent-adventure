...
(function (window, angular) {
   ...
   * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	...
	const $sanitizeMinError = angular.$$minErr('$sanitize');
	let bind;
	let extend;
	let forEach;
	let isDefined;
 ...
 *   `$compileProvider`}.
	...
	function $SanitizeProvider() {
		...
		this.$get = ['$$sanitizeUri', function ($$sanitizeUri) {
			...
			return function (html) {
				const buf = [];
				htmlParser(html, htmlSanitizeWriter(buf, (uri, isImage) => !$$sanitizeUri(uri, isImage).startsWith('unsafe:')));
				return buf.join('');
			};
		}];
   ...
   *   }
     ...
     */
      ...
      + 'xlink:type,xml:base,xml:lang,xml:space,xmlns,xmlns:xlink,y,y1,y2,zoomAndPan', true);
		...
		function toMap(string_, lowercaseKeys) {
			const object = {}; const items = string_.split(',');
			let i;
			for (i = 0; i < items.length; i++) {
				object[lowercaseKeys ? lowercase(items[i]) : items[i]] = true;
			}
     ...
     */
		const getInertBodyElement /* function(html: string): HTMLBodyElement */ = (function (window, document) {
			...
			if (document && document.implementation) {
				...
				throw $sanitizeMinError('noinert', 'Can\'t create an inert html document');
			}
   ...
   * });
     ...
     */
		function htmlParserImpl(html, handler) {
			...
			do {
				if (mXSSAttempts === 0) {
					throw $sanitizeMinError('uinput', 'Failed to sanitize html because the input is unstable');
				}
		...
		function encodeEntities(value) {
			return value
				...
				.replace(SURROGATE_PAIR_REGEXP, value => {
					...
					return '&#' + (((hi - 0xD8_00) * 0x4_00) + (low - 0xDC_00) + 0x1_00_00) + ';';
				})
     ...
     */
		...
		function getNonDescendant(propName, node) {
			...
			if (nextNode && nodeContains.call(node, nextNode)) {
				throw $sanitizeMinError('elclob', 'Failed to sanitize html because the element is clobbered: {0}', node.outerHTML || node.outerText);
			}
   ...
   */
	angular.module('ngSanitize').filter('linky', ['$sanitize', function ($sanitize) {
        ...
        = /((ftp|https?):\/\/|(www\.)|(mailto:)?[\w.%+-]+@)\S*[^\s.;,(){}<>"\u201D\u2019]/i;
		const MAILTO_REGEXP = /^mailto:/i;

		const linkyMinError = angular.$$minErr('linky');
		...
		return function (text, target, attributes) {
			...
			if (!isString(text)) {
				throw linkyMinError('notstring', 'Expected string but received: {0}', text);
			}
        ...
        = isFunction(attributes) ? attributes
			...
			while ((match = raw.match(LINKY_URL_REGEXP))) {
				...
				i = match.index;
				addText(raw.slice(0, Math.max(0, i)));
				addLink(url, match[0].replace(MAILTO_REGEXP, ''));
				raw = raw.slice(Math.max(0, i + match[0].length));
			}
