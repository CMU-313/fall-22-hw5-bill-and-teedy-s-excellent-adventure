...
(function (f) {
	...
	let define; let module; let exports; return (function e(t, n, r) {
		...
		var i = typeof require === 'function' && require; for (const element of r) {
			s(element);
		}
   ...
   */
		...
		if (options.onReady) {
			...
			if (!options.async) {
				...
				head = document.head || document.querySelectorAll('head')[0];
		...
		module.exports = {
			...
			// Strip the query-string
				...
				if (!styleNode.styleSheet) {
					styleNode.append(document.createTextNode(styles));
          ...
          && oldStyleNode.firstChild.nodeValue === styleNode.firstChild.nodeValue);
				...
				if (oldStyleNode === null || keepOldStyleNode === false) {
					const nextElement = sheet && sheet.nextSibling || null;
					if (nextElement) {
						nextElement.parentNode.insertBefore(styleNode, nextElement);
					} else {
		...
		module.exports = function (window, less, options) {
			function errorHTML(e, rootHref) {
				...
				const element = window.document.createElement('div'); let timer; let content; const
					errors = [];
				const filename = e.filename || rootHref;
				const filenameNoPath = filename.match(/([^/]+(\?.*)?)$/)[1];

				element.id = id;
				element.className = 'less-error-message';
        ...
        + '</h3>' + '<p>in <a href="' + filename + '">' + filenameNoPath + '</a> ';
				...
				const errorline = function (e, i, classname) {
					if (e.extract[i] !== undefined) {
						errors.push(template.replace(/{line}/, (Number.parseInt(e.line, 10) || 0) + (i - 1))
					...
					timer = setInterval(() => {
						...
						if (body) {
							if (document.getElementById(id)) {
								body.replaceChild(element, document.getElementById(id));
							} else {
	...
	/* global window, XMLHttpRequest */
			...
			FileManager.prototype.loadFile = function loadFile(filename, currentDirectory, options, environment, callback) {
				...
				if (options.useFileCache && fileCache[href]) {
					try {
						...
						callback({filename: href, message: 'Error loading file ' + href + ' error was ' + error.message});
					}
		...
		module.exports = function (window, options) {
			...
			function loadStyles(modifyVars) {
				...
				for (const style_ of styles) {
					style = style_;
					if (typePattern.test(style.type)) {
			...
			less.registerStylesheetsImmediately = function () {
				...
				for (const link of links) {
					if (link.rel === 'stylesheet/less' || (/stylesheet/.test(link.rel)
            && (typePattern.test(link.type)))) {
				...
				return new Promise((resolve, reject) => {
					...
					// Relies on less.sheets array, callback seems to be guaranteed to be called for every element of the array
						loadStyleSheets((e, css, _, sheet, webInfo) => {
							...
							browser.createCSS(window.document, css, sheet);
							less.logger.info('CSS for ' + sheet.href + ' generated in ' + (Date.now() - endTime) + 'ms');
		...
		const copyFromOriginal = function copyFromOriginal(original, destination, propertiesToCopy) {
			...
			for (const element of propertiesToCopy) {
				if (original.hasOwnProperty(element)) {
					destination[element] = original[element];
				}
			}
		};
 ...
 */
	...
	// Todo - do the same for the toCSS ?
		...
		const environment = function (externalEnvironment, fileManagers) {
			...
			for (const [i, propName] of functions.entries()) {
		...
		environment.prototype.getFileManager = function (filename, currentDirectory, options, environment, isSync) {
			...
			if (options.pluginManager) {
				fileManagers = [fileManagers].flat().concat(options.pluginManager.getFileManagers());
			}
		...
		function number(n) {
			if (n instanceof Dimension) {
				return Number.parseFloat(n.unit.is('%') ? n.value / 100 : n.value);
			}
		...
		function scaled(n, size) {
			if (n instanceof Dimension && n.unit.is('%')) {
				return Number.parseFloat(n.value * size / 100);
			}
        ...
        + (0.0722 * color.rgb[2] / 255);
			...
			// should be kept as is, so check for color
				...
				hsl.s += typeof method !== 'undefined' && method.value === 'relative' ? hsl.s * amount.value / 100 : amount.value / 100;
			...
			lighten(color, amount, method) {
				...
				hsl.l += typeof method !== 'undefined' && method.value === 'relative' ? hsl.l * amount.value / 100 : amount.value / 100;
			...
			fadein(color, amount, method) {
				...
				hsl.a += typeof method !== 'undefined' && method.value === 'relative' ? hsl.a * amount.value / 100 : amount.value / 100;
			...
			contrast(color, dark, light, threshold) {
				...
				threshold = typeof threshold === 'undefined' ? 0.43 : number(threshold);
        ...
        && (/^#([a-f\d]{6}|[a-f\d]{3})$/i.test(c.value))) {
			...
			functionRegistry.add('data-uri', function (mimetypeNode, filePathNode) {
				...
				if (!mimetypeNode) {
					...
					// Use base 64 unless it's an ASCII or UTF-8 format
						...
						useBase64 = !['US-ASCII', 'UTF-8'].includes(charset);
					}
					...
					useBase64 = mimetype.endsWith(';base64');
				}
				...
				const DATA_URI_MAX = 32_768;
				if (uri.length >= DATA_URI_MAX && this.context.ieCompat !== false) {
		...
		MathHelper._math = function (fn, unit, n) {
			...
			return new Dimension(fn(Number.parseFloat(n.value)), unit);
		};
		...
		mathFunctions.round = function (n, f) {
			...
			return mathHelper._math(number_ => number_.toFixed(fraction), null, n);
		};
		...
		functionRegistry.addMultiple({
			...
			convert(value, unit) {
				return value.convertTo(unit.value);
			},
			pi() {
			...
			percentage(n) {
				const result = mathHelper._math(number_ => number_ * 100, '%', n);
		...
		functionRegistry.addMultiple({
			e(string_) {
				return new Anonymous(string_ instanceof JavaScript ? string_.evaluated : string_.value);
			},
			escape(string_) {
				return new Anonymous(
					encodeURI(string_.value).replace(/=/g, '%3D').replace(/:/g, '%3A').replace(/#/g, '%23').replace(/;/g, '%3B')
			...
			'%'(string /* arg, arg, ... */) {
				...
				for (const arg of args) {
					...
					result = result.replace(/%[sda]/i, token => {
						const value = ((arg.type === 'Quoted')
            && /s/i.test(token)) ? arg.value : arg.toCSS();
		...
		functionRegistry.addMultiple({
          ...
          + (value instanceof Operation ? '. Have you forgotten parenthesis?' : '')};
				}

				if (unit) {
					unit = unit instanceof Keyword ? unit.value : unit.toCSS();
				} else {
					unit = '';
				}

				return new Dimension(value.value, unit);
			},
			'get-unit'(n) {
     ...
     */
		...
		module.exports = {
			error(message) {
				this._fireEvent('error', message);
			},
			warn(message) {
				this._fireEvent('warn', message);
			},
			info(message) {
				this._fireEvent('info', message);
			},
			debug(message) {
				this._fireEvent('debug', message);
			},
		...
		module.exports = function (SourceMapBuilder) {
			...
			ParseTree.prototype.toCSS = function (options) {
				...
				try {
					...
					throw new LessError(error, this.imports);
				}
				...
				if (options.pluginManager) {
					...
					for (const postProcessor of postProcessors) {
						result.css = postProcessor.process(result.css, {sourceMap: sourceMapBuilder, options, imports: this.imports});
					}
				}
			...
			var parse = function (input, options, callback) {
				...
				if (!callback) {
					...
					return new PromiseConstructor((resolve, reject) => {
						parse.call(self, input, options, (error, output) => {
							if (error) {
								reject(error);
							} else {
								resolve(output);
		...
		module.exports = function (input, fail) {
			const length = input.length; let level = 0; let parenLevel = 0;
			...
			function emitChunk(force) {
				const length_ = chunkerCurrentIndex - emitFrom;
				if (((length_ < 512) && !force) || !length_) {
					return;
				}
			...
			for (chunkerCurrentIndex = 0; chunkerCurrentIndex < length; chunkerCurrentIndex++) {
				...
				switch (cc) {
					...
					case 92: { // \
						if (chunkerCurrentIndex < length - 1) {
					...
					case 96: { // ", ' and `
						...
						for (chunkerCurrentIndex += 1; chunkerCurrentIndex < length; chunkerCurrentIndex++) {
					...
					case 47: { // /, check for comment
						if (parenLevel || (chunkerCurrentIndex == length - 1)) {
							continue;
						}
						...
						if (cc2 == 47) {
							// //, find lnfeed
							for (chunkerCurrentIndex += 2; chunkerCurrentIndex < length; chunkerCurrentIndex++) {
								cc2 = input.charCodeAt(chunkerCurrentIndex);
							...
							for (chunkerCurrentIndex += 2; chunkerCurrentIndex < length - 1; chunkerCurrentIndex++) {
								cc2 = input.charCodeAt(chunkerCurrentIndex);
								if (cc2 == 125) {
					...
					case 42: { // *, check for unmatched */
						if ((chunkerCurrentIndex < length - 1) && (input.charCodeAt(chunkerCurrentIndex + 1) == 47)) {
		...
		module.exports = function () {
			...
			parserInput.$quoted = function () {
				...
				for (let i = 1; i + currentPosition < length; i++) {
					...
					switch (nextChar) {
						...
						case startChar: {
							const string_ = input.substr(currentPosition, i + 1);
							skipWhitespace(i + 1);
							return string_;
						}
			...
			parserInput.start = function (string_, chunkInput, failFunction) {
				input = string_;
				...
				chunks = chunkInput ? chunker(string_, failFunction) : [string_];
		...
		const Parser = function Parser(context, imports, fileInfo) {
			...
			function expect(arg, message, index) {
				...
				error(message || (typeof arg === 'string' ? 'expected \'' + arg + '\' got \'' + parserInput.currentChar() + '\''
					: 'unexpected token'));
			}

			// Specialization of expect()
			function expectChar(arg, message) {
				if (parserInput.$char(arg)) {
					return arg;
				}

				error(message || 'expected \'' + arg + '\' got \'' + parserInput.currentChar() + '\'');
			}
			...
			return {
				...
				//
				parse(string_, callback, additionalData) {
					...
					if (context.pluginManager) {
						...
						for (const preProcessor of preProcessors) {
							string_ = preProcessor.process(string_, {context, imports, fileInfo});
						}
					}
					...
					string_ = preText + string_.replace(/^\uFEFF/, '') + modifyVars;
					imports.contents[fileInfo.filename] = string_;
					...
					try {
						parserInput.start(string_, context.chunkInput, function fail(message_, index) {
							throw new LessError({
								index,
								type: 'Parse',
								message: message_,
						...
						return callback(new LessError(error_, imports, fileInfo.filename));
					}
				...
				//
					primary() {
						const mixin = this.mixin; let root = []; let
							node;
              ...
              || mixin.call() || this.rulesetCall() || this.entities.call() || this.directive();
					...
					comment() {
						if (parserInput.commentStore.length > 0) {
					...
					//
						quoted() {
							let string_; const index = parserInput.i; let
								isEscaped = false;
							...
							return new (tree.Quoted)(string_.charAt(0), string_.slice(1, 1 + string_.length - 2), isEscaped, index, fileInfo);
						},
						...
						//
						keyword() {
							const k = parserInput.$char('%') || parserInput.$re(/^[_A-Za-z-][\w-]*/);
							if (k) {
              ...
              || parserInput.$re(/^(?:(?:\\[()'"])|[^()'"])+/) || '';
						...
						color() {
							...
							if (parserInput.currentChar() === '#' && (rgb = parserInput.$re(/^#([A-Fa-f\d]{6}|[A-Fa-f\d]{3})/))) {
							// Strip colons, brackets, whitespaces and other characters that should not
							// definitely be part of color string
								let colorCandidateString = rgb.input.match(/^#(\w+).*/);
								colorCandidateString = colorCandidateString[1];
								if (!/^[A-Fa-f\d]+$/.test(colorCandidateString)) { // Verify if candidate consists only of allowed HEX characters
						...
						colorKeyword() {
							...
							parserInput.autoCommentAbsorb = false;
							const k = parserInput.$re(/^[_A-Za-z-][\w-]+/);
						...
						unicodeDescriptor() {
							...
							ud = parserInput.$re(/^U\+[\da-fA-F?]+(-[\da-fA-F?]+)?/);
							if (ud) {
						...
						javascript() {
							...
							if (js) {
								parserInput.forget();
								return new (tree.JavaScript)(js.slice(0, Math.max(0, js.length - 1)), Boolean(escape), index, fileInfo);
							}
					...
					//
						call() {
							const s = parserInput.currentChar(); let important = false; const index = parserInput.i; let elementIndex;
							let elements; let element; let e; let c; let args;
							...
							while (true) {
								elementIndex = parserInput.i;
								e = parserInput.$re(/^[#.](?:[\w-]|\\(?:[A-Fa-f\d]{1,6} ?|[^A-Fa-f\d]))+/);
								if (!e) {
									break;
								}

								element = new (tree.Element)(c, e, elementIndex, fileInfo);
								if (elements) {
						...
						args(isCall) {
							...
							while (true) {
								...
								if (value_ && value_ instanceof tree.Variable) {
									if (parserInput.$char(':')) {
						...
						//
						definition() {
							let name; let parameters = []; let match; let ruleset; let cond; let
								variadic = false;
              ...
              || parserInput.peek(/^[^{]*}/)) {
							...
							match = parserInput.$re(/^([#.](?:[\w-]|\\(?:[A-Fa-f\d]{1,6} ?|[^A-Fa-f\d]))+)\s*\(/);
							if (match) {
								name = match[1];

								const argInfo = this.args(false);
								parameters = argInfo.args;
								...
								if (ruleset) {
									parserInput.forget();
									return new (tree.mixin.Definition)(name, parameters, ruleset, cond, variadic);
								}
            ...
            || parserInput.$re(/^(?:[.#]?|:*)(?:[\w-]|[^\u0000-\u009F]|\\(?:[A-Fa-f\d]{1,6} ?|[^A-Fa-f\d]))+/)
            ...
            || this.entities.variableCurly();
					...
					selector(isLess) {
						...
						while ((isLess && (extendList = this.extend())) || (isLess && (when = parserInput.$str('when'))) || (e = this.element())) {
							if (when) {
								...
								allExtends = allExtends ? allExtends.concat(extendList) : extendList;
							} else {
								if (allExtends) {
					...
					attribute() {
						...
						op = parserInput.$re(/^[|~*$^]?=/);
						if (op) {
							value = entities.quoted() || parserInput.$re(/^\d+%/) || parserInput.$re(/^[\w-]+/) || entities.variableCurly();
						}

						expectChar(']');

						return new (tree.Attribute)(key, op, value);
					},
					...
					mediaFeatures() {
						const entities = this.entities; const features = []; let
							e;
						do {
							e = this.mediaFeature();
							if (e) {
					...
					//
					operand() {
						const entities = this.entities; let
							negate;
            ...
            || entities.call() || entities.colorKeyword();
					...
					ruleProperty() {
						...
						const simpleProperty = parserInput.$re(/^([\w-]+)\s*:/);
						if (simpleProperty) {
   ...
   */
		...
		FunctionImporter.prototype.eval = function (contents, callback) {
			...
			try {
				...
				callback(new LessError({
					message: 'Plugin evaluation error: \'' + error.name + ': ' + error.message.replace(/"/g, '\'') + '\'',
		...
		module.exports = function (environment, ParseTree, ImportManager) {
			var render = function (input, options, callback) {
				...
				if (!callback) {
					...
					const self = this;
					return new PromiseConstructor((resolve, reject) => {
						render.call(self, input, options, (error, output) => {
							if (error) {
								reject(error);
							} else {
								resolve(output);
				...
				this.parse(input, options, (error, root, imports, options) => {
					if (error) {
		...
		module.exports = function (environment) {
			...
			SourceMapOutput.prototype.normalizeFilename = function (filename) {
				...
				if (this._sourceMapBasepath && filename.indexOf(this._sourceMapBasepath) === 0) {
					filename = filename.slice(this._sourceMapBasepath.length);
			...
			SourceMapOutput.prototype.add = function (chunk, fileInfo, index, mapLines) {
				...
				if (fileInfo) {
					...
					inputSource = inputSource.slice(0, Math.max(0, index));
		...
		const Assignment = function (key, value) {
			this.key = key;
			this.value = value;
		};
		...
		//
			if (Array.isArray(rgb)) {
				...
				this.rgb = rgb.match(/.{2}/g).map(c => Number.parseInt(c, 16));
			} else {
				this.rgb = rgb.split('').map(c => Number.parseInt(c + c, 16));
			}
      ...
      + '}line{font-family:\\00003' + ctx.debugInfo.lineNumber + '}}\n';
		...
		Dimension.prototype.genCSS = function (context, output) {
			...
			if (value !== 0 && value < 0.000_001 && value > -0.000_001) {
			// Would be output 1e-6 etc.
				stringValue = value.toFixed(20).replace(/0+$/, '');
			}
		...
		Dimension.prototype.convertTo = function (conversions) {
			...
			let i; let groupName; let group; let targetUnit; let derivedConversions = {}; let
				applyUnit;
			...
			/* jshint loopfunc:true */
				if (group.hasOwnProperty(atomicUnit)) {
					value = denominator ? value / (group[atomicUnit] / group[targetUnit]) : value * (group[atomicUnit] / group[targetUnit]);
		...
		Directive.prototype.eval = function (context) {
			let mediaPathBackup; let mediaBlocksBackup; let value = this.value; let
				rules = this.rules;
		...
		Directive.prototype.outputRuleset = function (context, output, rules) {
			...
			// Non-compressed
			const tabSetString = '\n' + new Array(context.tabLevel).join('  '); const
				tabRuleString = tabSetString + '  ';
			if (!ruleCnt) {
				output.add(' {' + tabSetString + '}');
			} else {
				output.add(' {' + tabRuleString);
		...
		Extend.prototype.findSelfSelectors = function (selectors) {
			...
			for (i = 0; i < selectors.length; i++) {
				...
				if (i > 0 && selectorElements.length > 0 && selectorElements[0].combinator.value === '') {
		...
		JsEvalNode.prototype.evaluateJavaScript = function (expression, context) {
			...
			try {
				...
				throw {message: 'JavaScript evaluation error: ' + error.message + ' from `' + expression + '`',
					filename: this.currentFileInfo.filename,
					index: this.index};
			}
			...
			try {
				...
				throw {message: 'JavaScript evaluation error: \'' + error.name + ': ' + error.message.replace(/"/g, '\'') + '\'',
					filename: this.currentFileInfo.filename,
					index: this.index};
			}
		...
		JsEvalNode.prototype.jsify = function (object) {
			if (Array.isArray(object.value) && (object.value.length > 1)) {
				return '[' + object.value.map(v => v.toCSS()).join(', ') + ']';
			}
		...
		Media.prototype.permute = function (array) {
			if (array.length === 0) {
				return [];
			}
			...
			const result = [];
			const rest = this.permute(array.slice(1));
			for (const element of rest) {
				for (let j = 0; j < array[0].length; j++) {
					result.push([array[0][j]].concat(element));
				}
			}
					...
					for (m = 0; m < candidates.length; m++) {
						...
						if ((candidate === defNone) || (candidate === defaultResult)) {
							try {
								...
								throw {message: error.message, index: this.index, filename: this.currentFileInfo.filename, stack: error.stack};
							}
						}
					}
      ...
      + (args ? args.map(a => {
      	...
      	argValue += a.value.toCSS ? a.value.toCSS() : '???';
		...
		const Definition = function (name, parameters, rules, condition, variadic, frames, visibilityInfo) {
			this.name = name;
			...
			this.required = parameters.reduce((count, p) => {
		...
		/* jshint boss:true */
			...
			let varargs; let arg;
			const parameters = this.params.slice(0);
			let i; let j; let value; let name; let isNamedFound; let argIndex; let
				argsLength = 0;
				...
				for (i = 0; i < argsLength; i++) {
					...
					if (name = (arg && arg.name)) {
						...
						for (j = 0; j < parameters.length; j++) {
							if (!evaldArguments[j] && name === parameters[j].name) {
				...
				if (name = parameters[i].name) {
					if (parameters[i].variadic) {
						...
						if (value) {
							...
							value = parameters[i].value.eval(mixinEnv);
							frame.resetCache();
						} else {
            ...
            + ' (' + argsLength + ' for ' + this.arity + ')'};
						...
						frame.prependRule(new Rule(name, value));
						evaldArguments[i] = value;
					}
				}

				if (parameters[i].variadic && args) {
					for (j = argIndex; j < argsLength; j++) {
		...
		Definition.prototype.matchArgs = function (args, context) {
			...
			// Check patterns
			length = Math.min(requiredArgsCnt, this.arity);

			for (let i = 0; i < length; i++) {
				if (!this.params[i].name && !this.params[i].variadic && args[i].value.eval(context).toCSS() != this.params[i].value.eval(context).toCSS()) {
					return false;
				}
			}
     ...
     and *any* other value for a != b (e.g. undefined, NaN, -2 etc.) */
      ...
      && !(b.type === 'Quoted' || b.type === 'Anonymous')) {
		...
		const Quoted = function (string_, content, escaped, index, currentFileInfo) {
			this.escaped = (escaped == null) ? true : escaped;
			this.value = content || '';
			this.quote = string_.charAt(0);
			this.index = index;
			this.currentFileInfo = currentFileInfo;
		};
		...
		Rule.prototype.genCSS = function (context, output) {
			...
			try {
				...
				error.filename = this.currentFileInfo.filename;
				throw error;
			}
		...
		Rule.prototype.eval = function (context) {
			let strictMathBypass = false; let name = this.name; let evaldValue; let
		...
		Ruleset.prototype.evalImports = function (context) {
			...
			for (i = 0; i < rules.length; i++) {
				if (rules[i].type === 'Import') {
					...
					if (importRules && (importRules.length > 0 || importRules.length === 0)) {
      ...
      && !lastSelector.condition.eval(
		...
		Ruleset.prototype.find = function (selector, self, filter) {
			...
			for (const rule of this.rulesets()) {
				if (rule !== self) {
		...
		Ruleset.prototype.genCSS = function (context, output) {
			...
			const tabRuleString = context.compress ? '' : new Array(context.tabLevel + 1).join('  ');
			const tabSetString = context.compress ? '' : new Array(context.tabLevel).join('  ');
			let sep;
			...
			// a selector, or {}.
			if (!this.root) {
				debugInfo = getDebugInfo(context, this, tabSetString);
		...
		Ruleset.prototype.joinSelector = function (paths, context, selector) {
			...
			function addReplacementIntoPath(beginningPath, addPath, replacedElement, originalSelector) {
				...
				// this also allows + a { & .b { .a & { ... though not sure why you would want to do that
					let combinator = replacedElement.combinator; const
						parentElement = addPath[0].elements[0];
					if (combinator.emptyOrWhitespace && !parentElement.combinator.emptyOrWhitespace) {
						combinator = parentElement.combinator;
					}

					// Join the elements so far with the first part of the parent
					newJoinedSelector.elements.push(new Element(combinator, parentElement.value, replacedElement.index, replacedElement.currentFileInfo));
			...
			// == [[.a] [.c]] [[.b] [.c]]
			//
				let i; let j; let k; let currentElements; let newSelectors; let selectorsMultiplied; let sel; let element; let hadParentSelector = false; let length; let lastSelector;
				...
				for (i = 0; i < inSelector.elements.length; i++) {
					element = inSelector.elements[i];
					// Non parent reference elements just get added
					if (element.value !== '&') {
						const nestedSelector = findNestedSelector(element);
						...
						// on to the current list of selectors to add
							...
							for (k = 0; k < nestedPaths.length; k++) {
								const replacementSelector = createSelector(createParenthesis(nestedPaths[k], element), element);
								addAllReplacementsIntoPath(newSelectors, [replacementSelector], element, inSelector, replacedNewSelectors);
							}
						...
						for (j = 0; j < newSelectors.length; j++) {
							...
							// it is not lost
								if (sel.length > 0) {
									sel[0].elements.push(new Element(element.combinator, '', element.index, element.currentFileInfo));
								}
							...
							// And the parent selectors
								...
								// then join the last selector's elements on to the parents selectors
									const newSelectorPath = addReplacementIntoPath(sel, context[k], element, inSelector);
		...
		Selector.prototype.createEmptySelectors = function () {
			const element = new Element('', '&', this.index, this.currentFileInfo);
			const sels = [new Selector([element], null, null, this.index, this.currentFileInfo)];
      ...
      && (this.elements[0].combinator.value === ' ' || this.elements[0].combinator.value === '');
		...
		Selector.prototype.eval = function (context) {
			...
			let elements = this.elements; let
		...
		const Unit = function (numerator, denominator, backupUnit) {
			...
			} else if (numerator && numerator.length > 0) {
		...
		// Dimension checks the unit is singular and throws an error if in strict math mode.
			...
			} else if (!strictUnits && this.denominator.length > 0) {
		...
		Unit.prototype.toString = function () {
			let i; let
				returnString = this.numerator.join('*');
			for (i = 0; i < this.denominator.length; i++) {
				returnString += '/' + this.denominator[i];
			}
		...
		Unit.prototype.isLength = function () {
			return Boolean(/px|em|%|in|cm|mm|pc|pt|ex/.test(this.toCSS()));
		};
		...
		const URL = function (value, index, currentFileInfo, isEvald) {
			this.value = value;
			this.currentFileInfo = currentFileInfo;
			this.index = index;
        ...
        && context.isPathRelative(value.value)) {
					if (!value.quote) {
				...
				value.value = context.normalizePath(value.value);

				// Add url args if enabled
				if (context.urlArgs && !/^\s*data:/.test(value.value)) {
					const delimiter = !value.value.includes('?') ? '?' : '&';
					const urlArgs = delimiter + context.urlArgs;
					if (value.value.includes('#')) {
						value.value = value.value.replace('#', urlArgs + '#');
					} else {
						value.value += urlArgs;
					}
				}
			...
			return new URL(value, this.index, this.currentFileInfo, true);
		};
		...
		Variable.prototype.find = function (object, fun) {
			for (var i = 0, r; i < object.length; i++) {
				r = fun.call(object, object[i]);
				if (r) {
					return r;
				}
			}
		...
		ExtendFinderVisitor.prototype = {
			...
			visitRuleset(rulesetNode, visitArgs) {
				...
				const rules = rulesetNode.rules; const
		...
		ProcessExtendsVisitor.prototype = {
			...
			checkExtendsForNonMatched(extendList) {
				...
				for (const extend of extendList.filter(extend => !extend.hasFoundMatches && extend.parent_ids.length == 1)) {
			...
			doExtendChaining(extendsList, extendsListTarget, iterationCount) {
				...
				for (extendIndex = 0; extendIndex < extendsList.length; extendIndex++) {
					for (targetExtendIndex = 0; targetExtendIndex < extendsListTarget.length; targetExtendIndex++) {
						...
						if (extend.parent_ids.includes(targetExtend.object_id)) {
							continue;
						}
						...
						if (matches.length > 0) {
							...
							for (const selfSelector of extend.selfSelectors) {
				...
				for (extendIndex = 0; extendIndex < allExtends.length; extendIndex++) {
					for (pathIndex = 0; pathIndex < rulesetNode.paths.length; pathIndex++) {
						...
						if (matches.length > 0) {
							...
							for (const selfSelector of allExtends[extendIndex].selfSelectors) {
			...
			isElementValuesEqual(elementValue1, elementValue2) {
				...
				if (elementValue1 instanceof tree.Selector) {
					...
					for (let i = 0; i < elementValue1.elements.length; i++) {
						if (elementValue1.elements[i].combinator.value !== elementValue2.elements[i].combinator.value && (i !== 0 || (elementValue1.elements[i].combinator.value || ' ') !== (elementValue2.elements[i].combinator.value || ' '))) {
							return false;
						}
		...
		ImportVisitor.prototype = {
			...
			processImportNode(importNode, context, importParent) {
				...
				try {
					...
					if (!error.filename) {
						error.index = importNode.index; error.filename = importNode.currentFileInfo.filename;
					}

					// Attempt to eval properly and treat as css
					importNode.css = true;
					// If that fails, this error will be thrown
					importNode.error = error;
				}
		...
		JoinSelectorVisitor.prototype = {
			...
			visitRuleset(rulesetNode, visitArgs) {
				...
				if (!rulesetNode.root) {
					...
					if (selectors) {
						...
						rulesetNode.selectors = selectors.length > 0 ? selectors : (selectors = null);
						if (selectors) {
			...
			visitDirective(directiveNode, visitArgs) {
				const context = this.contexts[this.contexts.length - 1];
				if (directiveNode.rules && directiveNode.rules.length > 0) {
		...
		Visitor.prototype = {
			...
			flatten(array, out) {
				...
				for (i = 0, cnt = array.length; i < cnt; i++) {
					item = array[i];
					if (item === undefined) {
						continue;
					}
   ...
   */
		...
		function asap(task) {
			let rawTask;
			rawTask = freeTasks.length > 0 ? freeTasks.pop() : new RawTask();
		...
		function handleResolved(self, deferred) {
			asap(() => {
				...
				const returnValue = tryCallOne(cb, self._55);
				if (returnValue === IS_ERROR) {
					reject(deferred.promise, LAST_ERROR);
				} else {
					resolve(deferred.promise, returnValue);
				}
			});
		}
   ...
   */
		...
		Promise.all = function (array) {
			const args = Array.prototype.slice.call(array);
			...
			return new Promise((resolve, reject) => {
				...
				function res(i, value) {
					if (value && (typeof value === 'object' || typeof value === 'function')) {
						if (value instanceof Promise && value.then === Promise.prototype.then) {
							while (value._65 === 3) {
								value = value._55;
							}

							if (value._65 === 1) {
								return res(i, value._55);
							}
						...
						const then = value.then;
						if (typeof then === 'function') {
							const p = new Promise(then.bind(value));
							p.then(value_ => {
								res(i, value_);
							}, reject);
							return;
		...
		if (typeof Promise.prototype.done !== 'function') {
			Promise.prototype.done = function (onFulfilled, onRejected) {
				const self = arguments.length > 0 ? Reflect.apply(this.then, this, arguments) : this;
				self.then(null, error => {
					setTimeout(() => {
						throw error;
					}, 0);
				});
