	...
	return {
		...
		link(scope, element) {
			...
			scope.$watch(() => nowTime() - fromTime, value => {
				angular.element(element).text(timeAgo.inWords(value, fromTime, scope.format));
			});
		},
	};
}]);
...
angular.module('yaru22.angular-timeago').factory('timeAgo', ['$filter', 'timeAgoSettings', function ($filter, timeAgoSettings) {
	...
	service.inWords = function (distanceMillis, fromTime, format, timezone) {
		const fullDateAfterSeconds = Number.parseInt(timeAgoSettings.fullDateAfterSeconds, 10);
		...
		if (timeAgoSettings.allowFuture && distanceMillis < 0) {
	...
	service.parse = function (input) {
		...
		if (/^\d+$/.test(input)) {
			return new Date(Number.parseInt(input, 10));
		}
